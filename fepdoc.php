...
if (!function_exists("oscar_agreg_menu_fep")) 
	...
	function valrequeride($valide){
		if($valide == 1){
			...
			echo 'readonly="readonly" style="border: 1px solid green"';
		}
			...
			$name = $current_user->user_login;
				...
				if ($resultadosform)
				{
					...
					foreach ($resultadosform as $formeditar) 
					{
				        ...
				        <label>Archivo 1:</label><input type="file" name="file_array[]" <?php valrequeride($formeditar->val1) ?>/><br /><br />
				        <?php
...
if(isset($_FILES['file_array'])){
    ...
    for($i = 0; $i < count($tmp_name_array); $i++){
        if(move_uploaded_file($tmp_name_array[$i], "test_uploads/".$idUsuario1.$Posicao.$name_array[$i])){
        	...
        	$sql = "INSERT INTO mk_documents (id_user, archivo1) VALUES ($idUsuario1, '$idUsuario1.$Posicao.$name_array[$i]')";
